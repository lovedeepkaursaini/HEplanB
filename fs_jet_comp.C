...
double lumis1[] = { 500, 500, 500, 500};
double lumis2[] = {150, 500};
...
void fs_jet_comp(int plotset=0, bool print=false, string psuff="png", string pdir="plots", TFile* file=NULL){
	//open file
	if(!file) file = TFile::Open("root://cmseos.fnal.gov//store/user/lovedeep/newestraddam/newDS/tree/tree_jet_pt30.root");
        TFile* fileMask = TFile::Open("root://cmseos.fnal.gov//store/user/lovedeep/newestraddam/Mask_iphi/tree/tree_jet_pt30.root");
        TFile* fileEEdark = TFile::Open("root://cmseos.fnal.gov//store/user/lovedeep/newestraddam/newDS/EEdarkoff/tree/tree_jet_pt30.root");
	...
	Color_t colors[] = {kBlack, kBlue, kMagenta+2, kRed, kCyan+2, kMagenta, kOrange+7, kYellow+3};
        ...
        if(plotset==0)
        {
           ...
           KGroup* group = new KGroup(colors[1],themarkers[1]);
           for(int ieta = 0; ieta < 8; ieta++){
              group->push_back(new KSample(file,(alg)2,years[1],lumis[1],etas8[ieta],etas8[ieta+1]));
           }
	...
	//option to do ALL THE PLOTS
		...
		for(int a = 1; a <= 2; a++){
			for(int y = 1; y < 2; y++){
				...
				for(int Lnum = 0; Lnum <= 3; Lnum++){
					...
					for(int ieta = 0; ieta < 8; ieta++){
                                                if( Lnum == 0 )      group->push_back(new KSample(fileEEdark,(alg)a,years[y],lumis1[Lnum],etas8[ieta],etas8[ieta+1]));
                                                else if ( Lnum == 1 )group->push_back(new KSample(file,(alg)a,years[2],lumis1[Lnum],etas8[ieta],etas8[ieta+1]));
                                                else if( Lnum == 2)  group->push_back(new KSample(file,(alg)a,years[y],lumis1[Lnum],etas8[ieta],etas8[ieta+1]));
						else if( Lnum ==3)   group->push_back(new KSample(fileMask,(alg)a,years[y],lumis1[Lnum],etas8[ieta],etas8[ieta+1]));
					}
					groups.push_back(group);
				}
		...
		for(int a = 1; a <= 2; a++){
			...
			for(int Lnum = 0; Lnum <= 1; Lnum++){				
				for(int y = 0; y <= 1; y++){
					KGroup* group = new KGroup(colors[Lnum],themarkers[y]);
					for(int ieta = 0; ieta < 8; ieta++){
						group->push_back(new KSample(file,(alg)a,years[y],lumis2[Lnum],etas8[ieta],etas8[ieta+1]));
					}
		...
		group->push_back(new KSample(file,PF,2019,500,2.85, 3.0,"",kBlack));
